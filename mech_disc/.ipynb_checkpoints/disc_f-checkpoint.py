...
#########################33333333333333333#####################################################
def mesh_brake_pad(min_mesh, max_mesh, filename, mesh_type):   
    import gmsh
    import sys
    import math
    import os
    ...
    rub_list = [rub1, rub2, rub3, rub4, rub5, rub6, rub7, 
                rub8, rub9, rub10, rub11, rub12, rub13, rub14, rub15, rub16, rub17, rub18]
    shell = gmsh.model.occ.fuse([(3, outer_pad)], [(3, rub_list[0])], 70) # 70 is just a name
    
    for i in range(len(rub_list) - 1): # shell is the combination volume
        shell = gmsh.model.occ.fuse([(3, 70 + i)], [(3, rub_list[i + 1])], 71 + i)
    gmsh.model.occ.synchronize()
    ...
    for rub in rublist:
        gmsh.model.addPhysicalGroup(2, (0, rub), rub-24) #(0,rub), 0 is empty tag.  
    # for the rubbing elements, P13 of UIC 541-3
    # Sinter material, 200 cm2, 18 rubbing elemets, r = 1.88 cm
    # Mesh size
    gmsh.option.setNumber("Mesh.MeshSizeMin", min_mesh)
